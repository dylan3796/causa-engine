...
## Interpretation — when nobody defined the outcomes

Not all outcomes arrive clearly defined; often the engine interprets what the systems of record show and proposes the definition. Doctrine, encoded as validation (`src/interpret/protocol.ts`): **interpretation proposes; it never settles.**

- **Requests** are built from observatory evidence (`causa interpret <engagement.json>`): one roster question when no actors are declared, one outcome-definition question per agent, each self-contained (question + evidence + samples).
- **Proposals** answer them — actor rosters and draft contracts only. Guardrails are validation, not convention: a proposed contract must sit at the Grade-D evidence floor (`rules` counterfactual — an interpreter asserting an experiment existed would be fabricating evidence), must bill as `usage` (pricing is a settlement term between humans), must keep the `interpreted-` id prefix (provenance stays visible), and must declare interpreter, confidence, and rationale.
- **Interpreters are pluggable behind the same gate.** `heuristic-v1` (deterministic, runs everywhere including the browser) answers from simple defensible rules — highest-volume joined output per agent, negation-lexicon quality bars. `scripts/interpret-claude.ts` is the LLM adapter — it lives OUTSIDE `src/` (the sealed core makes no network calls, hygiene-test-enforced: no `fetch`, no `node:http`) and its output faces the identical `validateProposals` gate. `ANTHROPIC_API_KEY=... npm run interpret:claude -- out/interpretation-requests.json`.
- **Confirmation is a human act**: review `interpretation.md`, DELETE unconfirmed proposals from the JSON, then `causa adopt <engagement.json> <proposals.json>` writes a new engagement file (the input is never mutated). Adoption derives the mechanics deterministically — claim mappings (every run of a contracted actor claims its outcome; verification does the filtering), the shared `interpreted-keys` extractor set, source labels, and the Tier-0 default verdict policy (RETIRE when nothing verifies, otherwise RENEGOTIATE on the evidence; impact = spend at stake) when the customer has defined none.
- **The org-sweep graduation** is the acceptance test (`tests/interpret.test.ts`): five heterogeneous exports with no contracts → observatory → interpret → adopt → a settled statement (1,700 claimed → 1,110 verified across three agents), with the duplicate-claim integrity flags correctly firing on the every-run-claims posture and sdr-bot's uncontracted `opportunity_created` events surfacing as the next proposal. The playground runs this same pipeline in the browser — "Adopt all proposals → settle on the ledger".
