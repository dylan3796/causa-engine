/**
 * Tier-0 engagement intake — "two exports and a join key" (CAUSA.md §6.5).
 *
 ...
 * the verdict rules. loadEngagement() turns it into EngineInputs + an
 * EngineConfig plus an intake report that accounts for every row — read,
 * mapped, or rejected with a reason. A pilot needs no integration project.
 */
...
import { mapRows, normalizeTimestamp, resolveField, resolveOptional, RowError, usdToCents, type FieldSpec } from "./mapping";
...
function readTable(path: string, format: "csv" | "ndjson"): TableReadResult {
...
function resolveSource(row: Parameters<typeof resolveField>[0], spec: FieldSpec): SourceId {
  ...
  if (!(SOURCE_IDS as readonly string[]).includes(value)) {
    throw new RowError(`unknown source "${value}" (expected one of ${SOURCE_IDS.join(", ")})`);
  }
...
export function loadEngagement(configPath: string): LoadedEngagement {
  ...
  return loadEngagementConfig(raw, baseDir);
}
...
export function loadEngagementConfig(engagement: EngagementConfig, baseDir: string): LoadedEngagement {
  ...
  const assumptions: string[] = [
    "Naive timestamps (no timezone) in exports are read as UTC.",
  ];
  ...
  for (const spec of engagement.activitySources) {
    ...
    const table = readTable(path, spec.format);
    let outsidePeriod = 0;

    const { records, rejects } = mapRows(table.rows, (row): ActivityRun => {
      const id = resolveField(row, spec.map.id, "run id");
      ...
      // A row claims an outcome when its workflowId resolves non-empty;
      // otherwise it is a non-claiming step (retries, prep, drafts).
      ...
      seenRunIds.add(id);
      return { id, source: resolveSource(row, spec.source), actorId: resolveField(row, spec.map.actorId, "actorId"),
        model: resolveOptional(row, spec.map.model, "model"), startedAt, endedAt, costCents, payload, claim };
    });
